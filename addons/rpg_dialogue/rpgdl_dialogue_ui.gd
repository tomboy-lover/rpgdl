...
@export_custom(PROPERTY_HINT_INPUT_NAME, "show_builtin") var skip_action: StringName = &"ui_accept"

@export_custom(PROPERTY_HINT_INPUT_NAME, "show_builtin") var show_hide_ui_action: StringName = &"ui_accept"
...
func _setup_default_ui() -> void:
	...
	_dialogue_ui_parent.anchor_top = 0.65
	...
	var dial_margin_size = min(_dialogue_ui_parent.size.y, _dialogue_ui_parent.size.x) / 16
	...
	(func(): 
		print("Executed safely at end of frame")
		print(_portrait_ui_texture.size)
		_portrait_ui_sprites.position = _portrait_ui_texture.position + (_portrait_ui_texture.size / 2) + Vector2(dial_margin_size, dial_margin_size)
	).call_deferred()
	print(_portrait_ui_texture.size)
	...
	_nametag_ui_parent.anchor_top = 0.55
	_nametag_ui_parent.anchor_right = 0.25
	_nametag_ui_parent.anchor_bottom = 0.65
	...
	_nametag_ui_label.add_theme_font_size_override("font_size", 24)
	...
	_choices_parent.anchor_top = 0.5 #0.34
	_choices_parent.anchor_right = 1.0 #0.99
	_choices_parent.anchor_bottom = 0.6 #0.54
